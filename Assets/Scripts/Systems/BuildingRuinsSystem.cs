...
partial struct BuildingRuinsSystem : ISystem {
    ...
    public void OnUpdate(ref SystemState state) {
        ...
        foreach ((
            RefRW<BuildingRuins> buildingRuins,
            ...
            buildingRuins.ValueRW.onUnitQueueChanged = true;
        }
        ...
        foreach ((
            ...
            if (buildingRuins.ValueRO.activeUnitType != spawnUnitTypeDynamicBuffer[0].unitType) {
                buildingRuins.ValueRW.activeUnitType = spawnUnitTypeDynamicBuffer[0].unitType;

                UnitTypeSO activeUnitTypeSO = 
                    GameAssets.Instance.unitTypeListSO.GetUnitTypeSO(buildingRuins.ValueRO.activeUnitType);

                buildingRuins.ValueRW.progressMax = activeUnitTypeSO.progressMax;
            }

            buildingRuins.ValueRW.progress += SystemAPI.Time.DeltaTime;

            if (buildingRuins.ValueRO.progress < buildingRuins.ValueRO.progressMax) {
                continue;
            }

            buildingRuins.ValueRW.progress = 0f;
            ...
            this.SpawnFormation(ref state, unitTypeSO, entitiesReferences, localTransform.ValueRO.Position, buildingRuins.ValueRO.rallyPositionOffset);
        }
    }
