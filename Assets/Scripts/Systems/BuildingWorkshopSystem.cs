...
partial struct BuildingWorkshopSystem : ISystem {
    ...
    public void OnUpdate(ref SystemState state) {
        ...
        foreach ((
            RefRW<BuildingWorkshop> buildingWorkshop,
        ...
        foreach ((
            ...
            if (buildingWorkshop.ValueRO.activeUnitType != spawnUnitTypeDynamicBuffer[0].unitType) {
                buildingWorkshop.ValueRW.activeUnitType = spawnUnitTypeDynamicBuffer[0].unitType;

                UnitTypeSO activeUnitTypeSO = 
                    GameAssets.Instance.unitTypeListSO.GetUnitTypeSO(buildingWorkshop.ValueRO.activeUnitType);

                buildingWorkshop.ValueRW.progressMax = activeUnitTypeSO.progressMax;
            }

            buildingWorkshop.ValueRW.progress += SystemAPI.Time.DeltaTime;

            if (buildingWorkshop.ValueRO.progress < buildingWorkshop.ValueRO.progressMax) {
                continue;
            }

            buildingWorkshop.ValueRW.progress = 0f;
            ...
            this.SpawnFormation(ref state, unitTypeSO, entitiesReferences, localTransform.ValueRO.Position, buildingWorkshop.ValueRO.rallyPositionOffset);
        }
    }
